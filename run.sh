...
OVA_URL='https://raw.githubusercontent.com/mondeja/solaris-vm-action/master/ova/sol-11_4-part[0-9].tar.gz'
MACHINE_IP=""
...
clean_ova_parts() {
  rm -f sol-11_4-part*.tar.gz
}

extract_ova_parts() {
  cat sol-11_4-part*.tar.gz | tar -xvf - -i
  clean_ova_parts
}
...
download_ova_parts() {
  ...
  curl -Z \
    --parallel-max 8 \
    https://raw.githubusercontent.com/mondeja/solaris-vm-action/master/ova/sol-11_4-part[0-39].tar.gz \
    -o 'sol-11_4-part#1.tar.gz'
}
...
sync_files() {
  sshpass -p "solaris" \
  rsync \
    --exclude _actions/vmactions/solaris-vm \
